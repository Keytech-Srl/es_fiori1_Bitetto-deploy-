	...
	(Controller, JSONModel, MessageBox, MessageToast) => {
		let controller = {
			...
			setStudent: function (oEvent) {
				...
				let selectedStudent = oEvent.getParameter("listItem").getBindingContext().getObject();
			...
			openStudentFormModal: async function (type) {
				this.studentFormModal ??= await this.loadFragment({ name: "esfiori1.view.fragments.StudentFormModal" });
			...
			deleteStudent: function (id) {
				this.getView().getModel().remove(`/Zet_test_Set('${id}')`, {
					success: function () {
